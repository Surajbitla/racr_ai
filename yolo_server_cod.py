...
try:
    while True:
        ...
        # Receive original image size length and data
        original_img_size_length_bytes = conn.recv(4)
        if not original_img_size_length_bytes:
            break
        original_img_size_length = int.from_bytes(original_img_size_length_bytes, 'big')
        original_img_size_str = conn.recv(original_img_size_length).decode('utf-8')  # Decode the mode
        original_img_size = tuple(map(int, original_img_size_str.split(',')))
        print(f"Received original image size: {original_img_size}")        
        ...
        print(f"Received split layer index: {split_layer_index}")
        ...
        if mode == "lossless":
            out = decompress_data_lossless(compressed_data)
        elif mode == "lossy":
            out = decompress_data_lossy(compressed_data)
        else:
