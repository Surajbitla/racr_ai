...
# Decompression method for lossy compressed data
def decompress_data_lossy_batched(compressed_data):
    """
    Decompression method for lossy compressed data using quantization.
    """
    # Deserialize the received data
    data_quantized, mins, maxs, original_img_size = pickle.loads(compressed_data)

    # Decompress by reversing the quantization
    data_decompressed = data_quantized * (maxs - mins) + mins
    return data_decompressed, original_img_size
...
try:
    while True:
        ...
        if mode == "lossless":
            ...
            out, original_img_size = decompress_data_lossy_batched(compressed_data)
        ...
        # Check if received_data is a NotDict instance
        if isinstance(out, NotDict):
            inner_dict = out.inner_dict
            for key in inner_dict:
                if isinstance(inner_dict[key], torch.Tensor):
                    inner_dict[key] = inner_dict[key].to(model.device)
                print(f"Key: {key}, Tensor Shape: {inner_dict[key].shape}, Device: {inner_dict[key].device}")
        else:
            print("Received data is not a NotDict")
            print(f"Type of received_data: {type(out)}")
            if isinstance(out, torch.Tensor):
                print(f"Tensor Shape: {out.shape}, Device: {out.device}")
            out = out.to(model.device)

        # Run the model on the decompressed data
        with torch.no_grad():
            try:
                # Call the model with the correct format
                # res, layer_outputs = model(received_data, start=split_layer_index)
                # If the model expects a dictionary input
                if isinstance(out, NotDict):
                    res, layer_outputs = model(**out.inner_dict, start=split_layer_index)
                else:
                    res, layer_outputs = model(out, start=split_layer_index)
