...
def compress_data_lossy(data, clevel):
    serialized_data = pickle.dumps(data)
    compressed_data = blosc2.compress(serialized_data, clevel=clevel, filter=blosc2.Filter.SHUFFLE, codec=blosc2.Codec.ZSTD)
...
# server_address = ('10.33.76.90', 12345)
...
def test_split_performance(client_socket, split_layer_index, mode, clevel=4):
    correct = 0
    total = 0
    ...
    with torch.no_grad():
        for input_tensor, original_image, image_files in tqdm(data_loader, desc=f"Testing split at layer {split_layer_index}"):
            ...
            if mode == "lossless":
                compressed_output, compressed_size = compress_data_lossless(out)
            elif mode == "lossy":
                compressed_output, compressed_size = compress_data_lossy(out, clevel)
            else:
            ...
            # Convert original_img_size to bytes
            original_img_size = original_image[0].size  # This returns a tuple (width, height)
            original_img_size_str = f"{original_img_size[0]},{original_img_size[1]}"  # Convert tuple to a string
            original_img_size_bytes = original_img_size_str.encode('utf-8')
            original_img_size_length = len(original_img_size_bytes)
            ...
            # Send original image size
            client_socket.sendall(original_img_size_length.to_bytes(4, 'big'))  # Send length of original image size first
            client_socket.sendall(original_img_size_bytes)  # Send mode string itself
    ...
    filename = f'{mode}_data_clevel_{clevel}.pkl'
...
host_time,travel_time,server_time,processing_time = test_split_performance(client_socket, split_layer_index, mode, clevel)
client_socket.close()
