...
# Blosc2 Lossy Compression using ZFP
def compress_data_lossy(data, clevel):
    serialized_data = pickle.dumps(data)
    # Perform lossy compression with ZFP codec
    compressed_data = blosc2.compress(
        serialized_data, 
        clevel=clevel, 
        filter=blosc2.Filter.SHUFFLE,  # Optional: Improves compression ratio
        codec=blosc2.Codec.ZFP,        # Use ZFP codec for lossy compression
        zfp_mode=blosc2.ZFPMode.ACCURACY,   # Set ZFP mode to control accuracy
        zfp_tolerance=1e-3             # Accuracy tolerance (adjust as needed)
    )
    size_bytes = sys.getsizeof(compressed_data)
    return compressed_data, size_bytes
    ...
    with torch.no_grad():
        for input_tensor, original_image, image_files in tqdm(data_loader, desc=f"Testing split at layer {split_layer_index}"):
            ...
            if mode == "lossless":
                ...
                compressed_output, compressed_size = compress_data_lossy(data_to_send, clevel)
            else:
